...
def blobs_50_outdir(tmp_path_factory: pytest.TempPathFactory) -> Path:
    ...
    run_fixture_pipeline(["blobs_50"], str(td))
    return td
...
def disconnected_200_outdir(tmp_path_factory: pytest.TempPathFactory) -> Path:
    ...
    run_fixture_pipeline(["disconnected_200"], str(td))
    return td


def test_step0_raw_data_output():
    with tempfile.TemporaryDirectory() as td:
        run_fixture_pipeline(["blobs_50"], td)
...
def test_pipeline_is_deterministic():
    with tempfile.TemporaryDirectory() as td_a, tempfile.TemporaryDirectory() as td_b:
        run_fixture_pipeline(["blobs_50"], td_a)
        run_fixture_pipeline(["blobs_50"], td_b)
        for fname in (
...
def test_step3_membership_no_explicit_zeros():
    ...
    with tempfile.TemporaryDirectory() as td:
        run_fixture_pipeline(datasets, td)
...
def test_step1_knn_exact_output():
    ...
    with tempfile.TemporaryDirectory() as td:
        run_fixture_pipeline(["blobs_50"], td, ["--knn-method", "exact"])
        p = Path(td) / "blobs_50"
...
def test_step1_knn_exact_distances_match_pairwise():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        for i in range(len(X)):
            for j in range(15):
                expected = dist_matrix[i, knn_indices[i, j]]
...
def test_knn_method_both_generates_all_files():
    ...
    with tempfile.TemporaryDirectory() as td:
        run_fixture_pipeline(["blobs_50"], td, ["--knn-method", "both"])
        p = Path(td) / "blobs_50"
        assert (p / "step1_knn.npz").exists()
...
def test_exact_path_deterministic():
    ...
    with tempfile.TemporaryDirectory() as td_a, tempfile.TemporaryDirectory() as td_b:
        run_fixture_pipeline(["blobs_50"], td_a, ["--knn-method", "exact"])
        run_fixture_pipeline(["blobs_50"], td_b, ["--knn-method", "exact"])
