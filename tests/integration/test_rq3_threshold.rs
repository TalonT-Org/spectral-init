...
fn rq3_threshold_override_subspace_agreement() {
    ...
    unsafe {
        std::env::set_var("SPECTRAL_DENSE_N_THRESHOLD", "2000");
    }
    let ((eigs_dense, vecs_dense), level_dense) = spectral_init::solve_eigenproblem_pub(
        &laplacian,
        n_components,
        42,
    ...
    assert_eq!(
        level_dense, 0,
        "expected Level 0 (dense EVD) with threshold=2000"
    );
    ...
    let ((eigs_lobpcg, vecs_lobpcg), level_lobpcg) = spectral_init::solve_eigenproblem_pub(
        &laplacian,
        n_components,
        42,
        spectral_init::ComputeMode::PythonCompat,
    ...
    assert!(
        level_lobpcg >= 1,
        "expected Level >= 1 (LOBPCG path) with threshold=50, got {level_lobpcg}"
    );
    ...
    assert!(
        results_dir.join("dense_n200_eigvecs.npy").exists(),
        "dense_n200_eigvecs.npy not written"
    );
    assert!(
        results_dir.join("lobpcg_n200_eigvecs.npy").exists(),
        "lobpcg_n200_eigvecs.npy not written"
    );
