...
- `--knn-method M`     KNN method: `both` (default), `approx`, or `exact`

Examples:

    # Generate all 7 datasets (both PyNNDescent and exact-distance paths)
    python tests/generate_fixtures.py
    ...
    # Generate only PyNNDescent fixtures (no _exact files)
    python tests/generate_fixtures.py --knn-method approx

    # Generate only exact-distance fixtures for small datasets
    python tests/generate_fixtures.py --knn-method exact
...
| `step1_knn_exact.npz` | `knn_indices`, `knn_dists`, `n_neighbors` | i32, f32, i32 | (n,k), (n,k), () | Exact KNN via `sklearn.pairwise_distances`; n < 4096 only |
| `step2_smooth_knn_exact.npz` | `sigmas`, `rhos`, `n_neighbors` | f32, f32, i32 | (n,), (n,), () | Bandwidth params from exact KNN distances |
| `step3_membership_exact.npz` | *(scipy sparse COO)* | f32 | (n,n) | Directed membership graph from exact KNN |
| `step4_symmetrized_exact.npz` | *(scipy sparse CSR)* | f32 | (n,n) | Fuzzy union from exact KNN; symmetric |
| `step5a_pruned_exact.npz` | *(scipy sparse CSR)* | f32 | (n,n) | Pruned graph from exact KNN; symmetric |
...
Python UMAP uses two different KNN algorithms depending on dataset size:

- For **n < 4096**: `UMAP.fit()` computes exact pairwise distances via
  `sklearn.metrics.pairwise_distances` and derives KNN from the full distance matrix.
- For **n ≥ 4096**: `UMAP.fit()` uses PyNNDescent (approximate nearest neighbors).

This threshold is sourced from `umap/umap_.py` in `lmcinnes/umap`.

To accurately represent what UMAP does internally, `generate_fixtures.py` runs **both paths**
by default for datasets with n < 4096, producing parallel `_exact.npz` fixture variants.

### Which datasets have exact-distance fixtures

| Dataset | n | Has `_exact` fixtures? |
|---------|---|------------------------|
| `blobs_50` | 50 | Yes |
| `blobs_500` | 500 | Yes |
| `moons_200` | 200 | Yes |
| `circles_300` | 300 | Yes |
| `near_dupes_100` | 100 | Yes |
| `disconnected_200` | 200 | Yes |
| `blobs_5000` | 5000 | **No** (n ≥ 4096) |

### Relationship to `full_umap_e2e.npz`

`full_umap_e2e.npz` is generated by `UMAP.fit_transform()` which internally uses the exact
pairwise-distance path for all 6 small datasets. The `_exact.npz` fixture variants make
this internal behavior explicit and testable at each pipeline step.

### Using `--knn-method`

Use `--knn-method` to generate only one path:

    # Only PyNNDescent fixtures (no _exact files, even for small datasets)
    python tests/generate_fixtures.py --knn-method approx

    # Only exact-distance fixtures (skips PyNNDescent; blobs_5000 gets only step0_raw_data)
    python tests/generate_fixtures.py --knn-method exact
