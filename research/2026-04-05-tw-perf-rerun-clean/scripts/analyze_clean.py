...
def section_h100k(dry_run):
    ...
    baseline_id, baseline_rec = find_record(forward_data, "tw_baseline", "10000")
    if baseline_rec is None:
        lines.append("  ERROR: baseline n=10K record not found in criterion_output.json.")
        return "\n".join(lines)
    ...
    for v in VARIANTS:
        vid, vrec = find_record(forward_data, BENCH_PREFIX[v], "10000")
        if vrec is None:
            lines.append(f"  WARN: {v} n=10K record not found; skipping.")
            pvals.append(1.0)
    ...
    lines += [
        f"  Baseline n=10K id: {baseline_id}",
    ...
    if reversed_data:
        ...
        for bprefix in ["tw_combined", "tw_baseline"]:
            _, frec = find_record(forward_data, bprefix, "10000")
            _, rrec = find_record(reversed_data, bprefix, "10000")
...
def section_h0_h1_clean(dry_run):
    """H0/H1-clean: step CPU-time fractions — baseline, n=10K, 30 iterations."""
    ...
    lines = ["## H0/H1-clean: Step CPU-Time Fractions (Baseline, n=10K)"]
    ...
    baseline_path = os.path.join(STEP_DIR, "gaussian_n10000_baseline.json")
