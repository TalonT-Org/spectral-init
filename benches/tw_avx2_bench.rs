...
fn bench_tw_avx2(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("tw_avx2");
    ...
    for n in [1000usize, 5000, 10000, 25000, 50000] {
        ...
        group.bench_with_input(BenchmarkId::new("avx2_kernel", n), &n, |b, _| {
            b.iter(|| spectral_init::trustworthiness_avx2_kernel(black_box(x.view()), black_box(y.view()), black_box(k)))
        });
    }
...
criterion_group!(benches, bench_tw_avx2);
