...
fn bench_tw_combined(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("tw_combined");
    ...
    for n in [1000usize, 5000, 10000, 25000, 50000] {
        ...
        group.bench_with_input(BenchmarkId::new("combined", n), &n, |b, _| {
            b.iter(|| spectral_init::trustworthiness_combined(black_box(x.view()), black_box(y.view()), black_box(k)))
        });
    }
...
criterion_group!(benches, bench_tw_combined);
